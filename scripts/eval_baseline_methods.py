...
def historical_average_predict(df, period=12 * 24 * 7, test_ratio=0.2, null_val=0.):
    ...
    for i in range(n_train, min(n_sample, n_train + period)):
        ...
        y_predict.iloc[i - n_train,
                       :] = historical[historical != null_val].mean()
    # Copy each period.
    for i in range(n_train + period, n_sample, period):
        size = min(period, n_sample - i)
        start = i - n_train
        y_predict.iloc[start:start + size, :] = y_predict.iloc[start -
                                                               period: start + size - period, :].values
...
def var_predict(df, n_forwards=(1, 3), n_lags=4, test_ratio=0.2):
    ...
    for input_ind in range(start, n_sample - n_lags):
        prediction = var_result.forecast(scaler.transform(
            df.values[input_ind: input_ind + n_lags]), max_n_forwards)
    ...
    df_predicts = []
    for i, n_forward in enumerate(n_forwards):
        df_predict = pd.DataFrame(scaler.inverse_transform(
            result[i]), index=df_test.index, columns=df_test.columns)
...
def eval_static(traffic_reading_df):
    ...
    for horizon in horizons:
        y_predict, y_test = static_predict(
            traffic_reading_df, n_forward=horizon, test_ratio=0.2)
        rmse = masked_rmse_np(preds=y_predict.to_numpy(),
                              labels=y_test.to_numpy(), null_val=0)
        mape = masked_mape_np(preds=y_predict.to_numpy(),
                              labels=y_test.to_numpy(), null_val=0)
        mae = masked_mae_np(preds=y_predict.to_numpy(),
                            labels=y_test.to_numpy(), null_val=0)
        line = 'Static\t%d\t%.2f\t%.2f\t%.2f' % (
            horizon, rmse, mape * 100, mae)
        logger.info(line)


def eval_historical_average(traffic_reading_df, period):
    y_predict, y_test = historical_average_predict(
        traffic_reading_df, period=period, test_ratio=0.2)
    rmse = masked_rmse_np(preds=y_predict.to_numpy(),
                          labels=y_test.to_numpy(), null_val=0)
    mape = masked_mape_np(preds=y_predict.to_numpy(),
                          labels=y_test.to_numpy(), null_val=0)
...
def eval_var(traffic_reading_df, n_lags=3):
    n_forwards = [3, 6, 12, 36]
    y_predicts, y_test = var_predict(traffic_reading_df, n_forwards=n_forwards, n_lags=n_lags,
                                     test_ratio=0.2)

    # print(y_predicts[0].to_numpy())
    # pd.DataFrame(data={'speed': y_predicts.to_numpy()}).to_csv("./pred.csv")
    # pd.DataFrame(data={'speed': y_test.to_numpy()}).to_csv("./gt.csv")
    logger.info('VAR (lag=%d)' % n_lags)
    logger.info('Model\tHorizon\tRMSE\tMAPE\tMAE')
    for i, horizon in enumerate(n_forwards):
        pd.DataFrame(y_predicts[i].to_numpy()).to_csv(
            f"./pred{horizon}.csv")
        pd.DataFrame(y_test.to_numpy()).to_csv(
            "./gt{}.csv")
        rmse = masked_rmse_np(
            preds=y_predicts[i].to_numpy(), labels=y_test.to_numpy(), null_val=0)
        mape = masked_mape_np(
            preds=y_predicts[i].to_numpy(), labels=y_test.to_numpy(), null_val=0)
        mae = masked_mae_np(preds=y_predicts[i].to_numpy(
        ), labels=y_test.to_numpy(), null_val=0)
